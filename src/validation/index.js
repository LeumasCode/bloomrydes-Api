...
class Validate {
  ...
  static validateUserLogin(req, res, next) {
    ...
    if (!phone) return allFieldsRequired(res, "phone");
    if (phone && !phone.length) return allFieldsRequired(res, "phone");
    if (token && !token.length) return allFieldsRequired(res, "token");
    next();
  }
